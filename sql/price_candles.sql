...
-- ---------------------------------------------------------------------------
-- Weekly candles  (rolled up from price_candles_1d by candle-aggregator)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS price_candles_1w (
    id          BIGSERIAL   PRIMARY KEY,
    coin_id     TEXT        NOT NULL,
    bucket      TIMESTAMPTZ NOT NULL,               -- start of the ISO week (Monday)
    open        NUMERIC(24, 8) NOT NULL,
    high        NUMERIC(24, 8) NOT NULL,
    low         NUMERIC(24, 8) NOT NULL,
    close       NUMERIC(24, 8) NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS uidx_candles_1w_coin_bucket
    ON price_candles_1w (coin_id, bucket);

CREATE INDEX IF NOT EXISTS idx_candles_1w_bucket
    ON price_candles_1w (bucket DESC);

-- ---------------------------------------------------------------------------
-- Monthly candles  (rolled up from price_candles_1w by candle-aggregator)
-- Used for 1y / 5y / max chart views.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS price_candles_1month (
    id          BIGSERIAL   PRIMARY KEY,
    coin_id     TEXT        NOT NULL,
    bucket      TIMESTAMPTZ NOT NULL,               -- first day of the month
    open        NUMERIC(24, 8) NOT NULL,
    high        NUMERIC(24, 8) NOT NULL,
    low         NUMERIC(24, 8) NOT NULL,
    close       NUMERIC(24, 8) NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS uidx_candles_1month_coin_bucket
    ON price_candles_1month (coin_id, bucket);

CREATE INDEX IF NOT EXISTS idx_candles_1month_bucket
    ON price_candles_1month (bucket DESC);
...
UNION ALL
SELECT 'price_candles_1w',     COUNT(*) FROM price_candles_1w
UNION ALL
SELECT 'price_candles_1month', COUNT(*) FROM price_candles_1month;
